...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Gridcoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "gridcoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "gridcoind.pid"));
...
void ShrinkDebugFile()
{
    ...
    if (file && GetFilesize(file) > 300000)
    {
...
void AddTimeData(const CNetAddr& ip, int64 nTime)
{
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        if (abs64(nMedian) < 35 * 60) // Gridcoin: changed maximum adjust to 35 mins to avoid letting peers change our time too much in case of an attack.
        {
            nTimeOffset = nMedian;
        }
        else
