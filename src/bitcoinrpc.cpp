...
bool ClientIsUsingLoopback(const boost::asio::ip::address& address)
{
	...
	if (WildcardMatch(strAddress, "0:0:0:0:0:0:0:1")) return true;
	if (WildcardMatch(strAddress, "::ffff:127.0.0.1")) return true;
     
	printf("CLIENT_IP %s",strAddress.c_str());

    if (address == asio::ip::address_v4::loopback()
     || address == asio::ip::address_v6::loopback()
     || (address.is_v4()  && (address.to_v4().to_ulong() & 0xff000000) == 0x7f000000)
     || (address.is_v4 () && (address.to_v4().to_ulong() == 0x7F000001))   
...
static Object JSONRPCExecOne(const Value& req, bool bLoopback)
{
    Object rpc_result;
...
static string JSONRPCExecBatch(const Array& vReq, bool bLoopback)
{
    Array ret;
    for (unsigned int reqIdx = 0; reqIdx < vReq.size(); reqIdx++)
        ret.push_back(JSONRPCExecOne(vReq[reqIdx],bLoopback));
...
json_spirit::Value CRPCTable::execute(const std::string &strMethod, const json_spirit::Array &params, bool bLoopback) const
{
    // Find method
